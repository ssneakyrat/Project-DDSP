...
def fft_convolve(audio,
    ...
    Modified to ensure device consistency.
    """
    ...
    if frame_size!=audio_size:
        # Create filters on the same device as audio
        filters = torch.eye(frame_size).unsqueeze(1).to(device)
    ...
    if frame_size!=audio_size:
        overlap_add_filter = torch.eye(audio_frames_out.size(-1), requires_grad=False).unsqueeze(1).to(device)
...
def apply_window_to_impulse_response(impulse_response,
    ...
    # Create window on the same device as impulse_response
    window = nn.Parameter(torch.hann_window(window_size), requires_grad=False).to(device)
